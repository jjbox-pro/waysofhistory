...
appl.loadMobileDependencies = function(){
	...
	utils.loadStylesheet('ggen/css/de/site/mobile/mobile_glagna_1614209556.css', {callback: function(){
		appl.onDependenciesLoaded();
	}});
	
	appl.dependencies++;
	
	utils.loadScript('ggen/js/de/site/mobile/app/appllogin_1608520207.js', {async: false, callback: function(){
		appl.onDependenciesLoaded();
	}});
};
...
appl.enterFromValid = {
    fields: {
        '.js-glagna-enterForm input[name=login]': {
            required: true,
            message: 'Der Name muss zwischen ' + glib.account.namelimit[0] + ' und ' + glib.account.namelimit[1] + '  Symbole enthalten',
            test: happy.rangeLength,
            arg: glib.account.namelimit
        },
        '.js-glagna-enterForm input[name=password]': {
            required: true,
            message: 'Passwort muss von ' + glib.account.passwordlimit[0] + ' bis ' + glib.account.passwordlimit[1] + '  Symbole enthalten',
            test: happy.rangeLength,
...
appl.checkName = function(){
    return 'Der Name muss zwischen ' + glib.account.namelimit[0] + ' und ' + glib.account.namelimit[1] + '  Symbole enthalten';
};
...
appl.regFromValid = {
    fields: {
        ...
        'input[name=rules]': {
            required: true,
            message: 'Die Vereinbarung und Speiregeln sind nicht akzeptiert',
			when: 'change',
            test: function() {
        ...
        'input[name=hard]': {
            required: true,
            message: 'Expertenstatus<br> ist nicht bestätigt!',
			when: 'change',
            test: function() {
...
appl.auth = {
	...
	errors: {
        6: 'Dieser Spieler ist nicht registriert.', //WEbadAccount
		7: 'Kann keinen Platz mit den angegeben Einstellungen für die Stadt finden', //WEcantLoadTown // не удалось подобрать место для города
        8: 'Dieser Hinsiedeln-Link ist schon benutzt',//WENodataинбд - при регистрации
        17: 'Name nicht angegeben.', //WEbadName //неправильная длина имени или имя совпадает с варваром. текст ошибки не нужно для этого кода
		18: 'Es ist nach Regeln verboten, mehrere Accounts in der Spielwelt zu haben', //WEalreadyUsed // недавно уже была регистрация
		23: 'Die angegebene E-Mail-Adresse wird bereits von einem anderen Account benutzt', //WEbadEmail
		24: function(){
			appl.toggleGenNameAnimation(true);
			return 'Es gibt schon ein solches Account. Denk aus oder <a class="link js-glagna-genName">erstelle</a> einen anderen Namen.';
		},
		25: 'Falscher Hinsiedlung-Link',
        31: function(data) {
            return wofh.interface.password ? 'Verboten. Dieser Account ist Account eines anderen Spielers' : 'Falsches Passwort. Vielleicht wurde das Register nicht eingehalten.';
        }, //WEbadPassword //неправильная длина пароля. тоже должна быть проверка еще на клиенте
        35: function(data) {
            var text = '';
            if (data) {
                if (data.time && data.reason) 
                    text += 'Der Account ist gesperrt und wird in ' + timeMgr.fPeriod(data.time, {format: Timer.format.full, noZero: true}) + ' entfernt. ' + data.reason + '.';
                else if (data.time) 
                    text += 'Zugang zum Account für ' + timeMgr.fPeriod(data.time, {format: Timer.format.full, noZero: true}) + ' gesperrt. ';
            } else
                text = 'Registrierung abgelehnt';
            
            return text;
        }, //WEnoAccess //Отказано в регистрации
        36: 'Seite erfrischen und nochmals eintreten', //WEbadKey //временный ключ устарел (авто редирект)
		37: 'Es gibt schon ein solches Account. Denk aus oder <a class="link js-glagna-genName">erstelle</a> einen anderen Namen.',
		45: 'Es gibt einen oder mehrere unzulässige Symbole im Text.',
		 //WEok3 //аккаунт с таким логином и паролем уже есть, предложить пользователю войти на него
		/*39: function() {
			return 'Некорректный код';
		},*/
        100: 'Kann nicht einloggen. Vielleicht ist dieser Spielwelt außer Betrieb',
    },
    
	errorsRecPass: {
        25: 'Zweite Anfrage abgelehnt',
    },
    
	errorsLuckmove: {
		0: "Die Glücksmünzen sind erfolgreich übergeben",
		6: "Es gibt kein solches Account in dieser Spielwelt",
		8: "Inkorrekter Link",
		18: "Der Link ist schon früher benutzt",
		35: "Es ist unmöglich, zwischen verschiedenen Spielzonen zu überweisen"
	},
