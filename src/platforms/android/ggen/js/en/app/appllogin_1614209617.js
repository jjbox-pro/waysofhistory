...
appl.loadMobileDependencies = function(){
	...
	utils.loadStylesheet('ggen/css/en/site/mobile/mobile_glagna_1614209556.css', {callback: function(){
		appl.onDependenciesLoaded();
	}});
	
	appl.dependencies++;
	
	utils.loadScript('ggen/js/en/site/mobile/app/appllogin_1608520207.js', {async: false, callback: function(){
		appl.onDependenciesLoaded();
	}});
};
...
appl.enterFromValid = {
    fields: {
        '.js-glagna-enterForm input[name=login]': {
            required: true,
            message: 'The name must be between ' + glib.account.namelimit[0] + ' and ' + glib.account.namelimit[1] + '  characters long',
            test: happy.rangeLength,
            arg: glib.account.namelimit
        },
        '.js-glagna-enterForm input[name=password]': {
            required: true,
            message: 'Password must contain from ' + glib.account.passwordlimit[0] + ' to ' + glib.account.passwordlimit[1] + '  symbols',
            test: happy.rangeLength,
...
appl.checkName = function(){
    return 'The name must be between ' + glib.account.namelimit[0] + ' and ' + glib.account.namelimit[1] + '  characters long';
};
...
appl.regFromValid = {
    fields: {
        ...
        'input[name=rules]': {
            required: true,
            message: 'The User agreement and Game Rules have not been accepted',
			when: 'change',
            test: function() {
        ...
        'input[name=hard]': {
            required: true,
            message: 'Expert status<br> is not confirmed!',
			when: 'change',
            test: function() {
...
appl.auth = {
	...
	errors: {
        6: 'No such player is registered.', //WEbadAccount
		7: 'Failed to find a city location with given properties', //WEcantLoadTown // не удалось подобрать место для города
        8: 'This joint colonization link has been used earlier',//WENodataинбд - при регистрации
        17: 'No name entered.', //WEbadName //неправильная длина имени или имя совпадает с варваром. текст ошибки не нужно для этого кода
		18: 'It&#39;s forbidden by the rules to have multiple accounts in the same game world', //WEalreadyUsed // недавно уже была регистрация
		23: 'This email is already being used by another account', //WEbadEmail
		24: function(){
			appl.toggleGenNameAnimation(true);
			return 'This account exists already. Think up or <a class="link js-glagna-genName">create</a> another name.';
		},
		25: 'Incorrect joint colonization link',
        31: function(data) {
            return wofh.interface.password ? 'Access denied. This account belongs to another player' : 'Incorrect password. Passwords are case-sensitive.';
        }, //WEbadPassword //неправильная длина пароля. тоже должна быть проверка еще на клиенте
        35: function(data) {
            var text = '';
            if (data) {
                if (data.time && data.reason) 
                    text += 'This account has been blocked and will be deleted in ' + timeMgr.fPeriod(data.time, {format: Timer.format.full, noZero: true}) + ' ' + data.reason + '.';
                else if (data.time) 
                    text += 'Access to this account will be blocked for ' + timeMgr.fPeriod(data.time, {format: Timer.format.full, noZero: true}) + ' ';
            } else
		...
		37: 'This account exists already. Think up or <a class="link js-glagna-genName">create</a> another name.',
		45: 'There&#39;s one or several unallowed symbols in the text.',
		 //WEok3 //аккаунт с таким логином и паролем уже есть, предложить пользователю войти на него
		/*39: function() {
			return 'Некорректный код';
		},*/
        100: 'Can&#39;t log in. This world might be down at the moment',
    },
    
	errorsRecPass: {
        25: 'Repeated request rejected',
    },
    
	errorsLuckmove: {
		0: "Luck Coins have been successfully transferred",
		6: "The account does not exist in this Game World",
		8: "Incorrect link",
		18: "The link has been used earlier",
		35: "It is impossible to make a transfer between different game zones"
	},
