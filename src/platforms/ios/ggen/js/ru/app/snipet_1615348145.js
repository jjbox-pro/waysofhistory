	...
	buildParams: function(slot, up, short){
		...
		switch (slot.getType()) {
			...
			case Build.type.hide: {
				...
				arr.push({title: 'Вместимость ', icon: 'slotCapacity', effect: effect});
			...
			case Build.type.grown: {
				...
				arr.push({title: 'Прирост населения', iconText: snip.growEffect(), effect: effect});
				...
				if( mult != 1 ){
					...
					arr.push({title: 'С модификациями', iconText: snip.growEffect(), effect: effect});
				}
			...
			case Build.type.administration: {
				...
				if( up ) {
					...
					effect = snip.resCount(Resource.ids.money, snip.fixedHint(slot.getAdminMoneyEffect())) + '<span> в час</span>';
				...
				if( up ) {
					...
					for (var res in resList.getList()){
						...
						arr.push({
							...
							effect: snip.resCount(res, snip.fixedHint(res.getCount())) + ' в час ' + snip.pop(slot.getPop().getCount(res.getId()))
						});
					}
		...
		if( slot.getUngrown() || (slot.canUp() && slot.getUp().getUngrown()) ){ 
			effect = snip.pop(snip.fixedHint(slot.getUngrown() * timeMgr.DtH)) + '<span> в день</span>';
		...
		if( slot.getType() == Build.type.prodBoost )
			arr.push({icon: 'abilities', effect: 'Бесполезно для научных зданий'});
