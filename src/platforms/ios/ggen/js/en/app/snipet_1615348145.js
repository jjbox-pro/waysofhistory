snip = {
	...
	accIcon: function(acc, opt){
		...
		return '<span class="iconAcc '+classAdd+'" '+(defAcc?'data-title="Player"':'')+'></span>';
	},
	...
	townLink: function(town, opt){
        ...
        if( !town || !(town.id || town.name) )
			town = new Town({id: 0, name: 'Unknown city'});
		...
		if( event ){
			...
			if( eventI ){
				...
				if( event.isBuild() ){
					...
					title += ' to ' + slot.getLevel() + ' level';
				
				title += '.<br>';
			}
			else{
				if( event.isRebuild() )
					title += '' + prevSlot.getName() + ' of ' + prevSlot.getLevel() + ' lvl. is being rebuilt into ' + slot.getName() + '';
				else if ( event.isDestroy() )
					title += '' + prevSlot.getName() + ' of ' + prevSlot.getLevel() + ' lvl. is being demolished entirely';
				else if ( event.isDestroylevel() )
					title += '' + prevSlot.getName() + ' is being demolished to ' + slot.getLevel() + ' lvl.';
				else{
					if ( prevSlot.isEmpty() )
						title += '' + slot.getName() + ' is being built';
					else
						title += '' + slot.getName() + ' is being upgraded to the level ' + slot.getLevel() + '';
				}
				
				title += '.<br>';
			}
			
			title += 'Ends at ' + timeMgr.fMoment(event.getTime()) + '';
		}
		else
	...
	slotTitle: function(slot){
		...
		if( slot.haveActions() && slot.getBldEvent().isCurBuild() ){
			...
			if( event.isRebuild() )
				title += '' + slot.getName() + ' of ' + slot.getLevel() + ' lvl. is being rebuilt into ' + slot.applyEvent().getName() + '';
			else if ( event.isDestroy() )
				title += '' + slot.getName() + ' of ' + slot.getLevel() + ' lvl. is being demolished entirely';
			else if ( event.isDestroylevel() )
				title += '' + slot.getName() + ' is being demolished to ' + (slot.getLevel()-1) + ' lvl.';
			else{
				if ( slot.getLevel() )
					title += '' + slot.getName() + ' is being upgraded to the level ' + slot.getUp().getLevel() + '';
				else
					title += '' + slot.getName() + ' is being built';
			}
			
			title += '.<br>Ends at ' + timeMgr.fMoment(event.getTime()) + '';
		} 
		else
	...
	slotLevelInfo: function(slot){
		...
		if (slot.isWonder()) {
			if( slot.level && slot.isActive() )
				...
				text += slot.getLevel()+' level. Not active';
		}
        else{
			text += slot.getLevel()+' level. ';
			
			if( slot.getPos() != Build.no )
				text += slot.getPos() + ' block';
		}
	...
	slotEventArrow: function(type){
		var title = '';
		switch(type){
			case 'destroy': title = 'Is being built'; break;
			case 'build': title = 'Is being built'; break;
			case 'rebuild': title = 'Is being built'; break;
		}
	...
	buildParamsEffect: function(val) {
		if (+val == 0) {
			...
			return ' (+' + val + ' ' + snip.hint(" After improvement") + ')';
		}
	},
	...
	slotTerrName: function(a) /*(slot), (terrain)*/{
		...
		var arr = ["Plain", "City center", "Hill", "Coast", "Perimeter", "Wonder of the World", ""];
		return arr[terrain];
	},
	...
	slotMaxLevel: function(slot){
		...
		return '<span class="slotLevel -type-'+type+'" data-title="Maximum level">'+slot.getMaxLevel()+'</span>';
	},
		...
		switch (error) { 
			...
			case Slot.buildErr.needBonus: 
				if( !slot || slot.isEmpty() )
					...
					return (wofh.events.isBldQueueEmpty() ? 'Upgrade to level' : 'Queue the upgrade to level') + ' ' + slot.getUp().getLevel();
			case Slot.buildErr.off: return 'Unable to upgrade, the structure is disabled';
			case Slot.buildErr.max: return 'Can&#39;t be upgraded, it&#39;s the maximal level available for this building';
			case Slot.buildErr.res: return slot.getLevel() == 1? 'Insufficient resources': 'Not enough resources for building';
			case Slot.buildErr.work: return 'Unable to upgrade, the structure is active';
			case Slot.buildErr.money: return 'Unable to upgrade, currency has not yet been invented';
			case Slot.buildErr.queue: return 'Construction queue is full';
		}
	},
	...
	slotDestroyIcon: function(slot){
		switch (slot.canDestroyErr()) {
			case Slot.buildErr.ok: return '<span class="buttonBuildSlot-icon -type-down -act-ok" data-title="Demolish"></span>';
			case Slot.buildErr.work: return '<span class="buttonBuildSlot-icon -type-down -act-no" data-title="Unable to demolish, the structure is active"></span>';
			case Slot.buildErr.queue: return '<span class="buttonBuildSlot-icon -type-down -act-wait" data-title="Construction queue is full"></span>';
	...
	slotSwapButton: function(slot){
        ...
        return '<a class="buttonBuildSlot js-slotbld-swap" data-title="Move">'+btn+'</a>';
	},
	...
	slotRebuildIcon: function(slot){
		switch (slot.canRebuildErr()) {
			...
			case Slot.buildErr.work: return '<span class="buttonBuildSlot-icon -type-rebuild -act-no" data-title="Unable to rebuild, the structure is active"></span>';
			case Slot.buildErr.queue: return '<span class="buttonBuildSlot-icon -type-rebuild -act-no" data-title="Construction queue is full"></span>';
			case Slot.buildErr.building: return '<span class="buttonBuildSlot-icon -type-rebuild -act-no" data-title="Unable to rebuild, the structure is being built"></span>';
			case Slot.buildErr.ok: return '<span class="buttonBuildSlot-icon -type-rebuild -act-ok" data-title="Rebuild"></span>';
			case Slot.buildErr.money: return '<span class="buttonBuildSlot-icon -type-rebuild -act-no" data-title="Unable to rebuild, currency has not yet been invented"></span>';
			case Slot.buildErr.res: return '<span class="buttonBuildSlot-icon -type-rebuild -act-no" data-title="Insufficient resources to rebuild"></span>';
		}
	},
	...
	scienceImgData: function(a)/*(science), (scienceId)*/{
		...
		if(science.getId() >= 1000) {
			...
			var imgId = 999;
			var imgName = 'No science is chosen';
			...
			var imgName = /*'Science is being researched '+*/science.getName();
		}
	...
	scienceBonus: function(type, value){
        ...
        switch (+type) {
            case Science.bonus.grown:
                return 'Population growth +' + utils.toInt(level * 24);
            case Science.bonus.culture:
                return 'Culture +' + level;
            case Science.bonus.income:
                return 'Income +' + utils.toPercent(level) + '%';
            case Science.bonus.knowledge:
                return 'Upgrading science distribution';
            case Science.bonus.war:
                return 'War +' + utils.toInt(level + Appl.C_EPSILON) + '% efficiency';
            case Science.bonus.production:
                return 'Production +' + utils.toPercent(level) + '%';
            case Science.bonus.view:
                return 'Visual range +' + utils.toInt(level + Appl.C_EPSILON);
            case Science.bonus.store:
                return 'Storage capacity +' + utils.toInt(level + Appl.C_EPSILON) + ' units.';
            case Science.bonus.trader:
                return 'Merchants +' + utils.toInt(level + Appl.C_EPSILON);
            case Science.bonus.fish:
                return 'Fish caught in all maritime cities +' + utils.toPercent(level) + '%';
            case Science.bonus.stone:
                return 'Granite production in all mountainous cities +' + utils.toPercent(level) + '%';
            case Science.bonus.iron:
                return 'Metal mining in all cities +' + utils.toPercent(level) + '%';
        }
    },

	scienceAbility: function(abilityId){
        switch (+abilityId) {
            case Science.ability.openMap: return 'Full map visibility';
            case Science.ability.money: return 'New economic system';
            case Science.ability.noBarter: return 'Barter trade obsolescence';
            case Science.ability.roadDirt: return 'Building dirt roads';
            case Science.ability.roadHighway: return 'Building highways';
            case Science.ability.roadRailway: return 'Building railways';
            case Science.ability.roadMaglev: return 'Building maglev road';
			case Science.ability.roadTunnel: return 'Tunnel building';
			
            case Science.ability.fish: return 'Fish production is possible';
            case Science.ability.cloth: return 'Clothes production is possible';
            case Science.ability.longTrade: return 'No limitation for trade distance';

            case Science.ability.tactics: return 'Battle coordination';
        }
	},
	...
	scienceIcons: function(science, opt){
		...
		if (science.deposits.length) {
			var hint = 'Unlocks deposits: ' + snip.list(science.deposits, function(deposit){return deposit.getName();});
		...
		for (var abilityId in science.abilities) {
			...
			switch (+abilityId) {
				...
				case Science.ability.roadTunnel:
					text += snip.icon(snip.c.anyIcon, 'mapImp', 'Road construction');
					break;
				default:
		...
		if ( science.specialistList.getLength() ) {
			text += snip.icon(snip.c.anyIcon, 'specialist', 'Great citizen');
		}
	...
	ratePoint: function(point, type, title){
		...
		switch(type){
			case 'atk': title = 'Attack rating points'; break;
			case 'def': title = 'Defence rating points'; break;
			case 'science': title = 'Knowledge rating points'; break;
			case 'prod': title = 'Production rating points'; break;
		};
	...
	luck: function(count, round){
		if (typeof(count) == 'undefined') return '<span class="iconLuck" data-title="Luck"></span>';
		else{
	...
	luckBig: function(opt){
		...
		var title = opt.noTitle ? '' : 'data-title="Luck"';
		
		return '<span class="iconLuckBig" ' + title + '></span>';
	},
	
	pop: function(count){
		if (typeof(count) == 'undefined') return '<span class="iconPop" data-title="Population"></span>';
		else return snip.nobr(snip.pop() + '' + (typeof(count) == 'number' ? utils.stages(count) : count));
	},
    
	death: function(count){
		if (typeof(count) == 'undefined') return '<span class="iconDeath" data-title="Population"></span>';
		else return snip.nobr(snip.death() + '' + (typeof(count) == 'number' ? utils.stages(count) : count));
	},
	
	trader: function(count){
		if (typeof(count) == 'undefined') return '<span class="iconTrader" data-title="Merchant"></span>';
	...
	tradeAlly: function(ally, opt){
		...
		if( opt.showOverflow ){
			...
			if( accOverflow > 0 ){
				text += ' ' + snip.alert('The number of participants in this Trade Union exceeds the permissible level of ' + snip.acc() + accOverflow + '.<br> The number of members is updated periodically');
	...
	tradeUnionCountry: function(country){
		...
		if( country )
			return '<span class="iconTradeUnionCountry" data-title="You are in this trade union as member of ' + utils.escapeHtml(snip.country(country)) + '"></span>';
		else
			return;
	},
	...
	cult: function(count){
		if (typeof(count) != 'undefined'){
			...
			return '<span class="iconCult" data-title="Cultural resources"></span>';
		}
	},
	...
	grow: function(){
		return '<span class="iconGrow" data-title="Demographic resources"></span>';
	},
	...
	depositClimateIcon: function(deposit){
		...
		var title = 'Colony climate — ' + Tile.climate[deposit.getClimate()].title;
	...
	sort: function(field, dir, cls){
        ...
        return "<span class='icsort js-sort " + active + " " + cls + "' data-dir='" + (~~dir) + "' data-field='" + field + "' data-title='Sort'></span>";
	},
	...
	ruler: function(){
        return '<span class="iconRuler" data-title="Distance"></span>';
    },
	
	pathIcon: function(title){
		title = title === undefined ? 'Route' : '';
    ...
    eventIconArmy: function(type, dir, branch, subtype){        
        ...
        var titles = {
            attack: {outgo:'Attack on enemy', incom:'Enemy attack', operation: ['Offensive military operation', 'Offensive military operation of the country']},
            defence: {outgo:'Outgoing reinforcements', incom:'Incoming reinforcements', operation: ['Defensive military operation', 'Defensive military operation of the country']},
		...
		if ( title ){
			if( dir == 'outgo' && subtype == 'noWork' )
				title += '. Except construction works';
		}
        title = title?'data-title="'+title+'"':'';
	...
	mapImpEffectProd: function(imp, opt){
		...
		return snip.icon(snip.c.anyIcon, 'maxWorkers', 'Maximum of workers in scientific buildings', false, 'vaB') + snip.wrp('-snip-impEffectProd', utils.formatNum(imp.calcEffectProd(opt.effect), formatOpt) + opt.postFix);
	},
    ...
    distance: function(path, opt){
        ...
        text += '(';
		text += '<span class="distLand" data-title="Land route'+(path.land?' '+utils.formatNum(path.land, {fixed:3}):'')+'">' + (path.land? Trade.roundDistance(path.land): '-') + '</span>';
		text += '&nbsp;/&nbsp;';
		text += '<span class="distWater" data-title="Deep water route'+(path.deepwater?' '+utils.formatNum(path.deepwater, {fixed:3}):'')+'">' + (path.deepwater? Trade.roundDistance(path.deepwater): '-') + '</span>';
		if (path.water) {
			text += '&nbsp/&nbsp;';
			text += '<span class="distWater" data-title="Shallow water route'+(path.water?' '+utils.formatNum(path.water, {fixed:3}):'')+'">' + Trade.roundDistance(path.water) + '</span>';
		}
        text += ')';
		text += '</snip>';
	...
	distPath: function(dist, path, opt){
		...
		if( path !== undefined ){
			...
			text += '(';
			text += '<snip class="paths-wrp">';
			text += '<span class="distLand" data-title="Optimal Path '+utils.formatNum(path, {fixed:3})+'">'+Trade.roundDistance(path)+'</span>';
			text += '</snip>';
			text += ')';
		}
    ...
    chatUser: function(status, cont){
    	if (status == ChatMgr.userStatus.user) {
    		...
    		var title = status == ChatMgr.userStatus.admin? 'Lord of this chat': 'Overseer of this chat';	
    		title = ' data-title="' + title + '"';
    	}
	...
	buildParams: function(slot, up, short){
		...
		switch (slot.getType()) {
			case Build.type.wonder: {
				...
				arr.push({
					title: 'Competing distance '+snip.hint('The area within which two identical Wonders of the World cannot be active'),
					icon: 'distance',
					effect: radius == 0? 'The entire world': radius
				});
			...
			case Build.type.embassy: {
				effect = utils.toInt(slot.getEffect())+ ' <span> points</span>';
			...
			case Build.type.hide: {
				effect = utils.toInt(slot.getEffect()) + '<span> units per resource</span>';
				
				if( slotUp )
					effect += ' ' + snip.buildParamsEffect(utils.toInt(slot.getEffectUp()));

				arr.push({title: 'Capacity ', icon: 'slotCapacity', effect: effect});

				break;
            }
			case Build.type.grown: {
				effect = snip.pop(snip.fixedHint(slot.getEffect() * timeMgr.DtH)) + '<span> per day</span>';
				
				if( slotUp )
					effect += ' '+snip.buildParamsEffect(snip.fixedHint(slot.getEffectUp() * timeMgr.DtH));

				arr.push({title: 'Population growth', iconText: snip.growEffect(), effect: effect});

				var mult = slotTown ? slotTown.calcGrownMult() : 1;
				
				if( mult != 1 ){
					effect = snip.pop(snip.fixedHint(slot.getEffect() * mult * timeMgr.DtH)) + '<span> per day</span>';
					
					if( slotUp )
						effect += ' ' + snip.buildParamsEffect(snip.fixedHint(slot.getEffectUp() * mult * timeMgr.DtH));
                    
					arr.push({title: 'With modifications', iconText: snip.growEffect(), effect: effect});
				}
			...
			case Build.type.administration: {
				...
				if( up ) {
					...
					effect = snip.resCount(Resource.ids.money, snip.fixedHint(slot.getAdminMoneyEffect())) + '<span> per hour</span>';
			...
			case Build.type.trade: {
				effect = utils.toInt(slot.getEffect()) + '<span> units.</span>';
			...
			case Build.type.corruption: {
				effect = '<span>by </span>' + snip.fixedHint(slot.getEffect()) + '<span> times</span>';
			...
			case Build.type.ecology: {
				effect = '<span>by </span>' + utils.toPercent(slot.getEffect()) + '<span> per day</span>';
			...
			case Build.type.production: {
				if( up ) {
					...
					if( effectWithMapImp )
						effect += '+' + snip.title(utils.formatNum(effectWithMapImp), 'Area improvement effect', 'tooltip-text');
					...
					for (var res in resList.getList()){
						...
						arr.push({
							...
							effect: snip.resCount(res, snip.fixedHint(res.getCount())) + ' per hour ' + snip.pop(slot.getPop().getCount(res.getId()))
						});
					}
			...
			case Build.type.prodBoost: {
				...
				if( slotTown ){
					...
					if( !up )
						prodBoostEffect = snip.tooltip(prodBoostEffect, 'For the current city');
		...
		if (up && slotTown.getAura().hasColossus()){
			...
			if (slot.getId() == wonderEffect.getBuild()) {
				effect = wonderEffect.getEffect() * slot.getLevel() + '<span> per hour</span>';
		...
		if( slot.getUngrown() || (slot.canUp() && slot.getUp().getUngrown()) ){ 
			effect = snip.pop(snip.fixedHint(slot.getUngrown() * timeMgr.DtH)) + '<span> per day</span>';
		...
		if( slot.getPay({useEconomy: up, withoutRace: !up}) || (slot.canUp() && slot.getUp().getPay({useEconomy: up, withoutRace: !up})) ){
			...
			effect = snip.resCount(Resource.ids.money, pay) + '<span> per hour</span>';
			...
			arr.push({
				title: snip.title(snip.c.anyIcon.rent.title, up?'Including saving':'', 'fakelink crH'),
				icon: 'rent',
				effect: effect
			});
		} 
		...
		if( slot.isWonder() ){
			...
			switch (slot.build.terrain) { 
				...
				case Build.terrain.flatWater: 
					effect = 'Plain near water';
					break; 
				case Build.terrain.flatNoWater: 
					effect = 'Plain, no water and no rivers';
					break; 
		...
		if( slot.getType() == Build.type.prodBoost )
			arr.push({icon: 'abilities', effect: 'No bonus for scientific buildings'});
	...
	checkboxBarter: function(opt){
		...
		opt.helperText = opt.helperText||[text+'disabled', text+'enabled'];
...
snip.c = {
    anyIcon: {
		...
		rent: {id: 0, title: 'Maintenance cost'},
        cult: {id: 3, title: 'Cultural limit of the city'},
        grow: {id: 6, title: 'Population growth rate of the city'},
		popKill: {id: 9},
		diplomacyPoints: {id: 10, title: 'Diplomacy Points'},
		countryInvite: {id: 10},
        distance: {id: 16, title: 'Distance'},
        maxWorkers: {id: 20, title: 'Max. workers'},
		retrain: {id: 20, title: 'Can be retrained into'},//icnb ic21
		clock: {id: 100},
		
		place: {id: 101, title: 'Training place'},//icnu ic2
		science: {id: 103, title: 'Science required'},//icnu ic4
		cost: {id: 104, title: 'Required for training'},//icnu ic5
		unitCapacity: {id: 105, title: 'Carrying capacity when plundering'},	//icnu ic7
		speed: {id: 106, title: 'Speed'},//icnu ic8
		features: {id: 108, title: 'Characteristics'},	//icnu ic10
		abilities: {id: 110, title: 'Characteristic'},//icnu ic12
		race: {id: 113, title: 'Race'},//icnu ic15
        antiAirDamage: {id: 114, title: 'Damage to aircraft'},
		deposit: {id: 117, title: 'Deposit'},
        mapImp: {id: 118, title: 'Area improvement'},
		cap: {id: 119, title: 'New possibility'},
		fuel: {id: 120, title: 'Resources for transport speed-up'},
        townDmg: {id: 121, title: 'Damage to city'},
		specialist:  {id: 219, title: 'Great citizen'},
		
		//строения
		maxLevel: {id: 1, title: 'Maximum level'},//icnb ic2
		trainSpeed: {id: 2, title: 'Training speed'},//icnb ic3
		waterTradeSpeed: {id: 2, title: 'Additional speed of merchants on water'},//icnb ic3
		waterArmySpeed: {id: 2, title: 'Warship speed'},//icnb ic3
		airArmySpeed: {id: 2, title: 'City’s air speed'},//icnb ic3
		maxCount: {id: 4, title: 'Maximum quantity'},//icnb ic5
		slotCapacity: {id: 5, title: 'Capacity'},	//icnb  ic6
		airDef: {id: 8, title: 'Damage to enemy air'},//icnb ic9
		trapDamage: {id: 9, title: 'Damage from traps'},//icnb ic10
		corruption: {id: 11, title: 'Decreases corruption'},//icnb ic12
		ecology: {id: 11, title: 'Improves the environment'},//icnb ic12
		economy: {id: 12, title: 'Economy'},	//icnb ic13
		traders: {id: 13, title: 'Merchants'},//icnb ic14
        ungrown: {id: 14, title: 'Population decrease'},//icnb ic15
		cityLocation: {id: 15, title: 'City location'},//icnb ic15
		slotLocationM: {id: 223, title: 'Hill'},// Mountain
		slotLocationW: {id: 23, title: 'Water'},// Water
		slotLocationF: {id: 123, title: 'Plain'},// Flat
		
		effect: {id: 17, title: 'Effect'},//icnb ic18
		workEffectivity: {id: 21, title: 'Labor effectivity'},//icnb ic22
		money: {id: 115},
		production: {id: 7, title: 'Production resource'},
		airDamage: {id: 114, title:'Damage to aircraft'}, //icnb ic16
		waterDamage: {id: 220, title:'Damage to ships'},
		empty: {id: 221},
		off: {id: 222},
    },
    ...
    bonusScience: {
		...
		grown: {id: 6, title: 'Population growth'},
		culture: {id: 3, title: 'Culture'},
		income: {id: 115, title: 'Income'},
		knowledge: {id: 103, title: 'Science distribution upgrade'},
		war: {id: 9},
		production: {id: 7, title: 'Production'},
		view: {id: 116, title: 'Range of view'},
		store: {id: 5, title: 'Basic storage capacity in the city'},
		trader: {id: 13, title: 'Merchants'},
		build: {id: 22},
		unit: {id: 122},
		unitPeace: {id: 124},
	},
	...
	townSpec: {
        ...
        townSpec1: {id: 1, title: 'Science'},
        townSpec2: {id: 2, title: 'Money'},
        townSpec3: {id: 3, title: 'Industry'},
        townSpec4: {id: 4, title: 'Army'},
        townSpec5: {id: 5, title: 'Increased importance'},
        townSpec6: {id: 6, title: 'Indispensable'},
        townSpec7: {id: 7, title: ''},
    },
	
    smenu: {
        cls: 'smenuIcon',
        ann: {id: 'ann', title: 'Announcements'},
        sim: {id: 'sim', title: 'Combat simulator'},
        hlp: {id: 'hlp', title: 'Encyclopedia'},
        opt: {id: 'opt', title: 'Settings'},
        frm: {id: 'frm', title: 'Community'},
        sup: {id: 'sup', title: 'Support'},
        ext: {id: 'ext', title: 'Exit'},
		fsc: {id: 'fsc', title: 'Fullscreen'},
    },
	...
	moneyIcon: {
		...
		inc: {id: '0', title: 'Income'},
		cons: {id: '1', title: 'Spendings'},
		stream: {id: '2', title: 'Supply'},
		tax: {id: '4', title: 'Taxes'},
		budget: {id: '6', title: 'Budget payout'},
		sum: {id: '3', title: 'Profit'},
	},
	...
	resProdtype: {
		...
		science: {id: 0, title: 'Science'},
		money: {id: 1, title: 'Financial'},
		production: {id: 3, title: 'Production'},
		food: {id: 2, title: 'Agricultural'},
	},
