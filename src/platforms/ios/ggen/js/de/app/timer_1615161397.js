function Timer(time, timezone, locTime, syncDelay){
	...
	this.getDimension = function(dimension){
		switch( dimension ){
			case Timer.dimension.d: return ' Tg.';
			case Timer.dimension.h: return ' St.';
			case Timer.dimension.m: return ' M.';
			case Timer.dimension.s: return ' S.';
			default: return '';
		}
	};
    ...
    this.fPeriodShort = function(time, noDimension){
        ...
        if (time < this.MtS){
            return timeMoment.format('s')+'S'; 
        }
        if (time < this.HtS){
            return timeMoment.format('m')+'M'; 
        }
        if (time < this.DtS){
            return timeMoment.format('H')+'St'; 
        }
        return Math.floor(time/this.DtS) + (!noDimension ? 'Tg.' : ''); 
    };
