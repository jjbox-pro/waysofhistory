snip = {
	...
	accIcon: function(acc, opt){
		...
		return '<span class="iconAcc '+classAdd+'" '+(defAcc?'data-title="Spieler"':'')+'></span>';
	},
	...
	townIcon: function(barb, rel, undf){
		...
		return '<img class="icon_town'+cls+'" src="img/_.gif">';
	},
	...
	townLink: function(town, opt){
        ...
        if( !town || !(town.id || town.name) )
			town = new Town({id: 0, name: 'Unbekannte Stadt'});
	...
	townHid: function(){
		return snip.townIcon()+" Verborgen";
	},
	...
	resListCheck: function(resList, town, bigIcon){
		town = town||wofh.town;
        
		var result = '',
            stock = town.getStock(),
            stockRes, has, title, warn, time, now = timeMgr.getNow(),
            iconFn = function(res, warn, title){
                return snip.resWarn(res, warn, title) + ' ';
            };
        
        if( bigIcon ){
            iconFn = function(res, warn, title){
                return snip.nobr(snip.resBig(res) + snip.title(snip.warnBg(utils.stages(res.getCount()), warn), title)) + ' ';
            };
        }
        ...
        resList.each(function(res){
            ...
            stockRes = stock.getElem(res);
            
			has = res.isFood() ? stock.getFoodHasAfterSpend(resList, now) : stockRes.getHasNow(now);
            
            if(res.getCount() <= has){
                title = 'Genug Ressourcen';
                warn = bigIcon ? snip.c.warn.green : snip.c.warn.no;
            } else if(res.getCount() > stock.getMax(res)){
                title = 'Nicht genug Kapazität. Verbessere die Lagerung-Gebäude!';
                warn = snip.c.warn.black;
            } else if(stockRes.updateHour <= 0 || res.isFood()){
                title = 'Nicht genug Ressource. Sammle, kaufe oder raube!';
                warn = snip.c.warn.red;
            } else {
                time = stockRes.calcTimeTo(res.getCount());
                title = 'Wird gesammelt in ' + timeMgr.fPeriod(time) + '';
                warn = snip.c.warn.brown;
            }
            
            result += iconFn(res, warn, title);
        });
	...
	resBigListCheck: function(resList, town){
        return snip.resListCheck(resList, town, true);
	},
	...
	resProdtypeName: function(prodType){
		switch (+prodType) {
			case 0: return "Forschung";
			case 1: return "Finanzen";
			case 2: return "Landwirtschaft";
			case 3: return "Industrie";
		}
	},
	...
	resProdtypeIcon: function(prodType, wide){
		...
		var iconEl = '<img class="resProdtype -type-' + icon + '" src="img/_.gif" title="' + title + '">';
    ...
    slotMini: function(slot, opt){
		...
		return '<a href="'+HashMgr.getHref('#/buildinfo/'+slot.getId())+'" class="build_icon_s '+(slot.isWonder()?'build_icon_sw':'')+' build_icon_s_mini" style="background-image:url(img/buildings2/'+slot.getId()+'_'+slot.getPic()+'.png)" data-title="'+opt.hint+'"></a>';
    },
	...
	slotEventTitle: function(slot, event, prevSlot){
		...
		if( event ){
			...
			if( eventI ){
				title = 'In Bauschlange auf ' + (eventI+1) + '. Position.<br>';
				
				title += event.isDestroy() || event.isRebuild() ? prevSlot.getName() : slot.getName();
				
				if( event.isRebuild() )
					title += ' ' + prevSlot.getLevel() + ' verbessert';
				
				title += ' — ' + Slot.actions[event.getAction()].title.toLowerCase();
				
				if( event.isBuild() ){
					if ( !prevSlot.isEmpty() )
						title += ' ' + slot.getLevel() + ' verbessert';
				}
				else if( event.isRebuild() )
					title += ' in ' + slot.getName();
				else if( event.isDestroylevel() )
					title += ' bis ' + slot.getLevel() + '. Level';
				
				title += '.<br>';
			}
			else{
				if( event.isRebuild() )
					title += '' + prevSlot.getName() + ' des ' + prevSlot.getLevel() + '. Levels wird umgebaut in ' + slot.getName() + '';
				else if ( event.isDestroy() )
					title += '' + prevSlot.getName() + ' des ' + prevSlot.getLevel() + '. Levels wird völlig zerstört';
				else if ( event.isDestroylevel() )
					title += '' + prevSlot.getName() + ' wird bis zum ' + slot.getLevel() + '. Level zerstört';
				else{
					if ( prevSlot.isEmpty() )
						title += '' + slot.getName() + ' wird gebaut';
					else
						title += '' + slot.getName() + ' wird auf Level ' + slot.getLevel() + ' verbessert';
				}
				
				title += '.<br>';
			}
			
			title += 'Endet ' + timeMgr.fMoment(event.getTime()) + '';
		}
		else
	...
	slotTitle: function(slot){
		...
		if( slot.haveActions() && slot.getBldEvent().isCurBuild() ){
			...
			if( event.isRebuild() )
				title += '' + slot.getName() + ' des ' + slot.getLevel() + '. Levels wird umgebaut in ' + slot.applyEvent().getName() + '';
			else if ( event.isDestroy() )
				title += '' + slot.getName() + ' des ' + slot.getLevel() + '. Levels wird völlig zerstört';
			else if ( event.isDestroylevel() )
				title += '' + slot.getName() + ' wird bis zum ' + (slot.getLevel()-1) + '. Level zerstört';
			else{
				if ( slot.getLevel() )
					title += '' + slot.getName() + ' wird auf Level ' + slot.getUp().getLevel() + ' verbessert';
				else
					title += '' + slot.getName() + ' wird gebaut';
			}
			
			title += '.<br>Endet ' + timeMgr.fMoment(event.getTime()) + '';
		} 
		else
	...
	slotLevelInfo: function(slot){
		...
		if (slot.isWonder()) {
			if( slot.level && slot.isActive() )
				...
				text += slot.getLevel()+' Level. Inaktiv';
		}
        else{
			text += slot.getLevel()+' Level. ';
			
			if( slot.getPos() != Build.no )
				text += slot.getPos() + '. Viertel';
		}
	...
	buildImgBigPath: function(slot, opt){
        ...
        return 'img/buildings2/'+slot.getId()+'_'+slot.getLevelPic(true)+'_'+slot.getImgSubtype()+blinkSuffix+'.png';
    },
	...
	slotArrow: function(){
		return '<img src="img/gui/quests/arrow.png">';
	},
	
	slotEventArrow: function(type){
		var title = '';
		switch(type){
			case 'destroy': title = 'Wird zerstört'; break;
			case 'build': title = 'Wird gebaut'; break;
			case 'rebuild': title = 'Wird umgebaut'; break;
		}
		return '<img src="img/_.gif" class="build-arrow -arrow-' + type + '" data-title="' + title + '">';
	},
	...
	buildParamsEffect: function(val) {
		if (+val == 0) {
			...
			return ' (+' + val + ' ' + snip.hint(" Nach der Verbesserung") + ')';
		}
	},
	...
	slotTerrName: function(a) /*(slot), (terrain)*/{
		...
		var arr = ["Flachland", "Stadtzentrum", "Hügel", "Ufer", "Umkreis", "Weltwunder", ""];
		return arr[terrain];
	},
	...
	slotMaxLevel: function(slot){
		...
		return '<span class="slotLevel -type-'+type+'" data-title="Maximales Level">'+slot.getMaxLevel()+'</span>';
	},
	...
	slotBuildIcon: function(slot, error){
		...
		var title = snip.slotBuildTitle(slot, error),
            cls = snip.slotBuildClass(slot, error);
	...
	slotStartBuildIcon: function(slot, error){
        if ( error === undefined )
			error = slot.canBuildErr();
		...
		switch (error) { 
			...
			case Slot.buildErr.needBonus: 
				if( !slot || slot.isEmpty() )
					...
					return (wofh.events.isBldQueueEmpty() ? 'Verbessern bis zum Level' : 'In die Reihenfolge für die Verbesserung bis zum Level') + ' ' + slot.getUp().getLevel();
			case Slot.buildErr.off: return 'Kann man nicht verbessern, das Gebäude ist abgeschaltet';
			case Slot.buildErr.max: return 'Verbesserung unmöglich, es ist maximale Level dieser Gebäude';
			case Slot.buildErr.res: return slot.getLevel() == 1? 'Nicht genug Ressourcen': 'Nicht genug Ressourcen für Bau';
			case Slot.buildErr.work: return 'Kann man nicht verbessern, das Gebäude wird verwendet';
			case Slot.buildErr.money: return 'Kann man nicht verbessern, Geld wurde noch nicht erfunden';
			case Slot.buildErr.queue: return 'Die Bau-Reihenfolge ist voll';
		}
	},
	...
	slotDestroyIcon: function(slot){
		switch (slot.canDestroyErr()) {
			case Slot.buildErr.ok: return '<span class="buttonBuildSlot-icon -type-down -act-ok" data-title="Abreißen"></span>';
			case Slot.buildErr.work: return '<span class="buttonBuildSlot-icon -type-down -act-no" data-title="Kann man nicht abreißen, das Gebäude wird verwendet"></span>';
			case Slot.buildErr.queue: return '<span class="buttonBuildSlot-icon -type-down -act-wait" data-title="Die Bau-Reihenfolge ist voll"></span>';
	...
	slotSwapButton: function(slot){
        ...
        return '<a class="buttonBuildSlot js-slotbld-swap" data-title="Bewegen">'+btn+'</a>';
	},
	...
	slotRebuildIcon: function(slot){
		switch (slot.canRebuildErr()) {
			...
			case Slot.buildErr.work: return '<span class="buttonBuildSlot-icon -type-rebuild -act-no" data-title="Kann man nicht umbauen, das Gebäude wird verwendet"></span>';
			case Slot.buildErr.queue: return '<span class="buttonBuildSlot-icon -type-rebuild -act-no" data-title="Die Bau-Reihenfolge ist voll"></span>';
			case Slot.buildErr.building: return '<span class="buttonBuildSlot-icon -type-rebuild -act-no" data-title="Kann man nicht umbauen, das Gebäude wird gebaut"></span>';
			case Slot.buildErr.ok: return '<span class="buttonBuildSlot-icon -type-rebuild -act-ok" data-title="Umbauen"></span>';
			case Slot.buildErr.money: return '<span class="buttonBuildSlot-icon -type-rebuild -act-no" data-title="Kann man nicht umbauen, Geld wurde nicht erfunden"></span>';
			case Slot.buildErr.res: return '<span class="buttonBuildSlot-icon -type-rebuild -act-no" data-title="Nicht genug Ressourcen für den Umbau"></span>';
		}
	},
	...
	buildImgPath: function(slot, opt) {
		...
		return ' style="background-image: url(img/buildings2/'+slot.getId()+'_'+slot.getLevelPic(opt.checkActions)+(opt.grey? '-gray':'')+'.png)" ';
	},
	...
	scienceImgData: function(a)/*(science), (scienceId)*/{
		...
		if(science.getId() >= 1000) {
			var imgId = -2;
			var imgName = 'Vereinigung';
		} else if (science.isEmpty()) {
			var imgId = 999;
			var imgName = 'Keine Wissenschaft gewahlt';
			...
			var imgName = /*'Wissenschaft wird erforscht '+*/science.getName();
		}
	...
	scienceImg: function(science)/*(science), (scienceId)*/{
		...
		return '<div class="scienceImg" style="background-image: url(img/help/sciences/'+science.id+'-.png)" data-title="'+science.name+'"></div>';
	},
	...
	scienceImgHover: function(science)/*(science), (scienceId)*/{
		...
		return '<div class="scienceImg" style="background-image: url(img/help/sciences/'+science.id+'-.png), url(img/help/sciences/'+science.id+'-.png)" data-title="'+science.name+'"></div>';
	},
	...
	scienceImgHoverBig: function(science)/*(science), (scienceId)*/{
		...
		return '<span class="mmenu-btnBig-type-science-img" style="background: url(img/help/sciences/'+science.id+'.png)" data-title="'+science.name+'"></span>';
	},
	
	scienceImgBig: function(science)/*(science), (scienceId)*/{
		science = snip.scienceImgData(science);
        
        return '<div class="scienceImgBig" style="background-image: url(img/help/sciences/'+science.id+'.png)" data-title="'+science.name+'"></div>';
    },
        
    scienceIcon: function(text){
        if (text) return snip.nobr(snip.scienceIcon() + text);
        return '<img class="event es" src="img/_.gif">';
    },
	...
	scienceBonus: function(type, value){
        ...
        switch (+type) {
            case Science.bonus.grown:
                return 'Bevölkerungswachstum +' + utils.toInt(level * 24);
            case Science.bonus.culture:
                return 'Kultur +' + level;
            case Science.bonus.income:
                return 'Einkommen +' + utils.toPercent(level) + '%';
            case Science.bonus.knowledge:
                return 'Verbesserung der wissenschaftlichen Verteilung';
            case Science.bonus.war:
                return 'Krieg +' + utils.toInt(level + Appl.C_EPSILON) + '% Effizienz';
            case Science.bonus.production:
                return 'Produktion +' + utils.toPercent(level) + '%';
            case Science.bonus.view:
                return 'Sichtradius +' + utils.toInt(level + Appl.C_EPSILON);
            case Science.bonus.store:
                return 'Kapazität des Lagershauses +' + utils.toInt(level + Appl.C_EPSILON) + ' Einh.';
            case Science.bonus.trader:
                return 'Händler +' + utils.toInt(level + Appl.C_EPSILON);
            case Science.bonus.fish:
                return 'Fischfang in jeder Hafenstadt +' + utils.toPercent(level) + '%';
            case Science.bonus.stone:
                return 'Granitgewinnung in jeder Stadt auf Hügeln +' + utils.toPercent(level) + '%';
            case Science.bonus.iron:
                return 'Metallgewinnung in jeder Stadt +' + utils.toPercent(level) + '%';
        }
    },

	scienceAbility: function(abilityId){
        switch (+abilityId) {
            case Science.ability.openMap: return 'Vollständige Sicht dir Karte';
            case Science.ability.money: return 'Neues Wirtschaftssystem';
            case Science.ability.noBarter: return 'Veralterung des Tauschhandels';
            case Science.ability.roadDirt: return 'Bau von Feldwegen';
            case Science.ability.roadHighway: return 'Bau einer Autobahn';
            case Science.ability.roadRailway: return 'Bau einer Eisenbahn';
            case Science.ability.roadMaglev: return 'Bau einer Magnetschwebebahn';
			case Science.ability.roadTunnel: return 'Tunnelbau';
			
            case Science.ability.fish: return 'Fisch-Produktion ermöglicht';
            case Science.ability.cloth: return 'Kleidung-Produktion ermöglicht';
            case Science.ability.longTrade: return 'Keine Begrenzung von Handeldistanz';

            case Science.ability.tactics: return 'Koordination des Kampfs';
        }
	},
	...
	scienceIcons: function(science, opt){
		...
		if (science.deposits.length) {
			var hint = 'Öffnet Vorkommen: ' + snip.list(science.deposits, function(deposit){return deposit.getName();});
		...
		for (var abilityId in science.abilities) {
			...
			switch (+abilityId) {
				...
				case Science.ability.roadTunnel:
					text += snip.icon(snip.c.anyIcon, 'mapImp', 'Straßenbau');
					break;
				default:
		...
		if ( science.specialistList.getLength() ) {
			text += snip.icon(snip.c.anyIcon, 'specialist', 'Groß Bürger');
		}
	...
	ratePoint: function(point, type, title){
		...
		switch(type){
			case 'atk': title = 'Bewertungspunkte von Angriff'; break;
			case 'def': title = 'Bewertungspunkte von Verteidigung'; break;
			case 'science': title = 'Bewertungspunkte von Wissen'; break;
			case 'prod': title = 'Bewertungspunkte von Produktion'; break;
		};
	...
	ratePointIcon: function(type, title){
		...
		title = title||'Bewertungspunkte';
		
		var titles = {
            science: ' von Wissenschaft',
			prod: ' von Production',
			atk: ' von Angriff',
			def: ' von Verteidigung',
        };
	...
	luck: function(count, round){
		if (typeof(count) == 'undefined') return '<span class="iconLuck" data-title="Glück"></span>';
		else{
	...
	luckBig: function(opt){
		...
		var title = opt.noTitle ? '' : 'data-title="Glück"';
		
		return '<span class="iconLuckBig" ' + title + '></span>';
	},
	
	pop: function(count){
		if (typeof(count) == 'undefined') return '<span class="iconPop" data-title="Bevölkerung"></span>';
		else return snip.nobr(snip.pop() + '' + (typeof(count) == 'number' ? utils.stages(count) : count));
	},
    
	death: function(count){
		if (typeof(count) == 'undefined') return '<span class="iconDeath" data-title="Bevölkerung"></span>';
		else return snip.nobr(snip.death() + '' + (typeof(count) == 'number' ? utils.stages(count) : count));
	},
	
	trader: function(count){
		if (typeof(count) == 'undefined') return '<span class="iconTrader" data-title="Händler"></span>';
	...
	tradeAlly: function(ally, opt){
		...
		if( opt.showOverflow ){
			...
			if( accOverflow > 0 ){
				text += ' ' + snip.alert('Die Anzahl der Teilnehmer in diesem Handelsbündnis überschreitet das zulässige Niveau von ' + snip.acc() + accOverflow + '.<br>Die Anzahl der Mitglieder wird regelmäßig aktualisiert');
	...
	tradeUnionCountry: function(country){
		...
		if( country )
			return '<span class="iconTradeUnionCountry" data-title="Sie sind Mitglied dieses ' + utils.escapeHtml(snip.country(country)) + '-Handelsbündnis"></span>';
		else
			return;
	},
	...
	cult: function(count){
		if (typeof(count) != 'undefined'){
			...
			return '<span class="iconCult" data-title="Kultur-Ressourcen"></span>';
		}
	},
	...
	grow: function(){
		return '<span class="iconGrow" data-title="Demographische Ressourcen"></span>';
	},
	...
	depositClimateIcon: function(deposit){
		...
		var title = 'Vorkommenklima — ' + Tile.climate[deposit.getClimate()].title;
	...
	sort: function(field, dir, cls){
        ...
        return "<span class='icsort js-sort " + active + " " + cls + "' data-dir='" + (~~dir) + "' data-field='" + field + "' data-title='Sortieren'></span>";
	},
	...
	fixedHint: (function(){
        ...
        return function(num, opt, optInit, optExt, optTooltip){
            optText = {};
            ...
            return snip.tooltip((optText.formatFunc||utils.formatNum)(num, optText), (optHint.formatFunc||utils.formatNum)(num, optHint), optTooltip);
        };
    })(),
	
	ruler: function(){
        return '<span class="iconRuler" data-title="Distanz"></span>';
    },
	
	pathIcon: function(title){
		title = title === undefined ? 'Weg' : '';
    ...
    eventIconArmy: function(type, dir, branch, subtype){        
        ...
        var titles = {
            attack: {outgo:'Angriff auf den Feind', incom:'Angriff des Feindes', operation: ['Angriff-Militär-Operation', 'Angriff-Militär-Operation des Landes']},
            defence: {outgo:'Ausgehende Unterstützung', incom:'Ankommende Unterstützung', operation: ['Verteidigung-Militär-Operation', 'Verteidigung-Militär-Operation des Landes']},
            reinf: 'Unterstützungen',
        };
	...
	mapImpEffectProd: function(imp, opt){
		...
		return snip.icon(snip.c.anyIcon, 'maxWorkers', 'Maximum der Arbeiter in wissenschaftlichen Gebäuden', false, 'vaB') + snip.wrp('-snip-impEffectProd', utils.formatNum(imp.calcEffectProd(opt.effect), formatOpt) + opt.postFix);
	},
    ...
    distance: function(path, opt){
        ...
        text += '(';
		text += '<span class="distLand" data-title="Landweg'+(path.land?' '+utils.formatNum(path.land, {fixed:3}):'')+'">' + (path.land? Trade.roundDistance(path.land): '-') + '</span>';
		text += '&nbsp;/&nbsp;';
		text += '<span class="distWater" data-title="Weg durch tiefes Gewässer'+(path.deepwater?' '+utils.formatNum(path.deepwater, {fixed:3}):'')+'">' + (path.deepwater? Trade.roundDistance(path.deepwater): '-') + '</span>';
		if (path.water) {
			text += '&nbsp/&nbsp;';
			text += '<span class="distWater" data-title="Weg durch flaches Gewässer'+(path.water?' '+utils.formatNum(path.water, {fixed:3}):'')+'">' + Trade.roundDistance(path.water) + '</span>';
		}
        text += ')';
		text += '</snip>';
	...
	distPath: function(dist, path, opt){
		...
		if( path !== undefined ){
			...
			text += '(';
			text += '<snip class="paths-wrp">';
			text += '<span class="distLand" data-title="Optimaler Pfad '+utils.formatNum(path, {fixed:3})+'">'+Trade.roundDistance(path)+'</span>';
			text += '</snip>';
			text += ')';
		}
	...
	smile: function(smile, code, cls){
		...
		return '<img class="smile -type-'+cls+' -id-'+code+'"  data-type="'+cls+'" data-id="'+code+'" src="img/_.gif" >';
	},
    ...
    chatUser: function(status, cont){
    	if (status == ChatMgr.userStatus.user) {
    		...
    		var title = status == ChatMgr.userStatus.admin? 'Der Herr dieses Chats': 'Aufseher dieses Chats';	
    		title = ' data-title="' + title + '"';
    	}
	...
	postIcon: function(postId){
        ...
        return '<img class="iconPost -type-' + postId + '" src="img/_.gif" data-title="' + post.title + '">';
	},
	...
	buildParams: function(slot, up, short){
		...
		if( !up && !slot.getRaces().hasAll() ){
			...
			arr.push({icon: 'race',	effect: effect});	
		}
		...
		switch (slot.getType()) {
			case Build.type.wonder: {
				...
				arr.push({
					title: 'Konkurrenz-Entfernung '+snip.hint('Entfernung, in der es kein gleiches aktives Weltwunder geben kann'),
					icon: 'distance',
					effect: radius == 0? 'Die ganze Welt': radius
				});
			...
			case Build.type.train: {
				effect = snip.fixedHint(slot.getEffect(), {toPercent: true}) + '%';
                
				if( slotUp )
					effect += ' ' + snip.buildParamsEffect(snip.fixedHint(slot.getEffectUp(), {toPercent: true}) + '%');

				arr.push({icon: 'trainSpeed', effect: effect});

				break; 
            }
			case Build.type.embassy: {
				effect = utils.toInt(slot.getEffect())+ ' <span> Punkte</span>';
				
				if( slotUp )
					effect += ' ' + snip.buildParamsEffect(slot.getEffectUpInt());

				arr.push({icon: 'diplomacyPoints', effect: effect});
			...
			case Build.type.store: {
				...
				arr.push({icon: 'slotCapacity', effect: effect});
                
				break;
            }
			case Build.type.hide: {
				effect = utils.toInt(slot.getEffect()) + '<span> Einh. jeder Ressource</span>';
				
				if( slotUp )
					effect += ' ' + snip.buildParamsEffect(utils.toInt(slot.getEffectUp()));

				arr.push({title: 'Kapazität ', icon: 'slotCapacity', effect: effect});

				break;
            }
			case Build.type.grown: {
				effect = snip.pop(snip.fixedHint(slot.getEffect() * timeMgr.DtH)) + '<span> pro Tag</span>';
				
				if( slotUp )
					effect += ' '+snip.buildParamsEffect(snip.fixedHint(slot.getEffectUp() * timeMgr.DtH));

				arr.push({title: 'Zuwachs der Bevölkerung', iconText: snip.growEffect(), effect: effect});

				var mult = slotTown ? slotTown.calcGrownMult() : 1;
				
				if( mult != 1 ){
					effect = snip.pop(snip.fixedHint(slot.getEffect() * mult * timeMgr.DtH)) + '<span> pro Tag</span>';
					
					if( slotUp )
						effect += ' ' + snip.buildParamsEffect(snip.fixedHint(slot.getEffectUp() * mult * timeMgr.DtH));
                    
					arr.push({title: 'Mit Modifikationen', iconText: snip.growEffect(), effect: effect});
				}
			...
			case Build.type.administration: {
				/*культура*/
				effect = snip.fixedHint(slot.getAdminCultEffect());
                ...
                effect = snip.fixedHint(slot.getAdminEconEffect(), {toPercent: true}) + '%';;
                
				if( slotUp )
					effect += ' ' + snip.buildParamsEffect(snip.fixedHint(slot.getAdminEconEffectUp(), {toPercent: true}) + '%');
                
				arr.push({icon: 'economy', effect: effect});
                
				if( up ) {
					/*сумма экономии*/
                    
					effect = snip.resCount(Resource.ids.money, snip.fixedHint(slot.getAdminMoneyEffect())) + '<span> pro Stunde</span>';
                    
					if( slotUp )
						effect += ' ' + snip.buildParamsEffect(snip.fixedHint(slot.getAdminMoneyEffectUp()));
                    
					arr.push({title: '', icon: 'economy', effect: effect});
				}
				
				break; 
            }
			case Build.type.trade: {
				effect = utils.toInt(slot.getEffect()) + '<span> St.</span>';
			...
			case Build.type.corruption: {
				effect = '<span>um</span>' + snip.fixedHint(slot.getEffect()) + '<span></span>';
			...
			case Build.type.ecology: {
				effect = '<span>um </span>' + utils.toPercent(slot.getEffect()) + '<span> pro Tag</span>';
			...
			case Build.type.production: {
				if( up ) {
					...
					if( effectWithMapImp )
						effect += '+' + snip.title(utils.formatNum(effectWithMapImp), 'Landschaftsverbesserubgeffekt', 'tooltip-text');
					...
					arr.push({icon: 'maxWorkers', effect: effect});
					...
					for (var res in resList.getList()){
						...
						arr.push({
							...
							effect: snip.resCount(res, snip.fixedHint(res.getCount())) + ' pro Stunde ' + snip.pop(slot.getPop().getCount(res.getId()))
						});
					}
			...
			case Build.type.prodBoost: {
				...
				if( slotTown ){
					...
					if( !up )
						prodBoostEffect = snip.tooltip(prodBoostEffect, 'Für die aktuelle Stadt');
		...
		if (up && slotTown.getAura().hasColossus()){
			...
			if (slot.getId() == wonderEffect.getBuild()) {
				effect = wonderEffect.getEffect() * slot.getLevel() + '<span> pro Stunde</span>';
		...
		if( slot.getUngrown() || (slot.canUp() && slot.getUp().getUngrown()) ){ 
			effect = snip.pop(snip.fixedHint(slot.getUngrown() * timeMgr.DtH)) + '<span> pro Tag</span>';
			
            if( slotUp )
				effect += ' ' + snip.buildParamsEffect(snip.pop(snip.fixedHint(slot.getUngrownUp() * timeMgr.DtH)));

			arr.push({icon: 'ungrown', effect: effect});
		}
		...
		if( slot.getPay({useEconomy: up, withoutRace: !up}) || (slot.canUp() && slot.getUp().getPay({useEconomy: up, withoutRace: !up})) ){
			...
			effect = snip.resCount(Resource.ids.money, pay) + '<span> pro Stunde</span>';
			...
			arr.push({
				title: snip.title(snip.c.anyIcon.rent.title, up?'Unter Berücksichtigung der Ökonomie':'', 'fakelink crH'),
				icon: 'rent',
				effect: effect
			});
		} 
		...
		if( !up && !(slot.isWonder() && slot.getId() != Slot.ids.machuPicchu) )
			arr.push({icon: 'maxCount',	effect: slot.getMaxCount()});
		...
		if( slot.isWonder() ){
			...
			switch (slot.build.terrain) { 
				...
				case Build.terrain.flatWater: 
					effect = 'Flachland neben der Wasserstelle';
					break; 
				case Build.terrain.flatNoWater: 
					effect = 'Flachland ohne Wasserstelle und Flüssen';
					break; 
				...
				case Build.terrain.anyWater: 
					effect = 'Neben der Wasserstelle';
					break; 
			}
			
			arr.push({icon: 'cityLocation',	effect: effect});
		}
		else if( !up ){
			var icon;
		...
		if( slotTown ){
			...
			arr.push({icon: 'maxLevel',	effect: effect});
		}
		
		if( slot.getType() == Build.type.prodBoost )
			arr.push({icon: 'abilities', effect: 'Gibt kein Bonus den wissenschaftlichen Gebäuden'});
	...
	checkboxBarter: function(opt){
		...
		opt.helperText = opt.helperText||[text+'deaktiviert', text+'aktiviert'];
...
snip.c = {
    anyIcon: {
		...
		rent: {id: 0, title: 'Unterhalt'},
        cult: {id: 3, title: 'Kultur-Limit dieser Stadt'},
        grow: {id: 6, title: 'Wachstum der Bevölkerung dieser Stadt'},
		popKill: {id: 9},
		diplomacyPoints: {id: 10, title: 'Diplomatie-Punkte'},
		countryInvite: {id: 10},
        distance: {id: 16, title: 'Entfernung'},
        maxWorkers: {id: 20, title: 'Max. Arbeiter'},
		retrain: {id: 20, title: 'Man kann umlernen in'},//icnb ic21
		clock: {id: 100},
		
		place: {id: 101, title: 'Trainingsplatz'},//icnu ic2
		science: {id: 103, title: 'Benötigte Wissenschaft'},//icnu ic4
		cost: {id: 104, title: 'Für Ausbildung benotigt'},//icnu ic5
		unitCapacity: {id: 105, title: 'Tragfähigkeit bei Plünderung'},	//icnu ic7
		speed: {id: 106, title: 'Geschwindigkeit'},//icnu ic8
		features: {id: 108, title: 'Eigenschaften'},	//icnu ic10
		abilities: {id: 110, title: 'Besonderheit'},//icnu ic12
		race: {id: 113, title: 'Rasse'},//icnu ic15
        antiAirDamage: {id: 114, title: 'Schaden an Luftflotte'},
		deposit: {id: 117, title: 'Vorkommen'},
        mapImp: {id: 118, title: 'Landschaftsverbesserung'},
		cap: {id: 119, title: 'Neue Möglichkeit'},
		fuel: {id: 120, title: 'Ressourcen für Handelbeschleunigung'},
        townDmg: {id: 121, title: 'Schaden an die Stadt'},
		specialist:  {id: 219, title: 'Groß Bürger'},
		
		//строения
		maxLevel: {id: 1, title: 'Maximales Level'},//icnb ic2
		trainSpeed: {id: 2, title: 'Trainings-Geschwindigkeit'},//icnb ic3
		waterTradeSpeed: {id: 2, title: 'Zusätzliche Geschwindigkeit der Händler auf Wasser'},//icnb ic3
		waterArmySpeed: {id: 2, title: 'Geschwindigkeit der Kampfschiffe'},//icnb ic3
		airArmySpeed: {id: 2, title: 'Geschwindigkeit der Luftflotte der Stadt'},//icnb ic3
		maxCount: {id: 4, title: 'Maximale Anzahl'},//icnb ic5
		slotCapacity: {id: 5, title: 'Kapazität'},	//icnb  ic6
		airDef: {id: 8, title: 'Schaden an die feindliche Luftflotte'},//icnb ic9
		trapDamage: {id: 9, title: 'Schaden durch Fallen'},//icnb ic10
		corruption: {id: 11, title: 'Verringert die Korruption'},//icnb ic12
		ecology: {id: 11, title: 'Verbessert die Ökologie'},//icnb ic12
		economy: {id: 12, title: 'Ökonomie'},	//icnb ic13
		traders: {id: 13, title: 'Händler'},//icnb ic14
        ungrown: {id: 14, title: 'Bevölkerungsrückgang'},//icnb ic15
		cityLocation: {id: 15, title: 'Stadtlage'},//icnb ic15
		slotLocationM: {id: 223, title: 'Hügel'},// Mountain
		slotLocationW: {id: 23, title: 'Wasser'},// Water
		slotLocationF: {id: 123, title: 'Flachland'},// Flat
		
		effect: {id: 17, title: 'Effekt'},//icnb ic18
		workEffectivity: {id: 21, title: 'Effektivität der Arbeit'},//icnb ic22
		money: {id: 115},
		production: {id: 7, title: 'Produktionsressource'},
		airDamage: {id: 114, title:'Schaden an Luftflotte'}, //icnb ic16
		waterDamage: {id: 220, title:'Schaden an Schiffe'},
		empty: {id: 221},
		off: {id: 222},
    },
    ...
    bonusScience: {
		...
		grown: {id: 6, title: 'Bevölkerungswachstum'},
		culture: {id: 3, title: 'Kultur'},
		income: {id: 115, title: 'Einkommen'},
		knowledge: {id: 103, title: 'Verbesserung der wissenschaftlichen Verteilung'},
		war: {id: 9},
		production: {id: 7, title: 'Produktion'},
		view: {id: 116, title: 'Sichtbereich'},
		store: {id: 5, title: 'Grundlegende Speicherkapazität in der Stadt'},
		trader: {id: 13, title: 'Händler'},
		build: {id: 22},
		unit: {id: 122},
		unitPeace: {id: 124},
	},
	...
	townSpec: {
        ...
        townSpec1: {id: 1, title: 'Wissenschaft'},
        townSpec2: {id: 2, title: 'Geld'},
        townSpec3: {id: 3, title: 'Industrie'},
        townSpec4: {id: 4, title: 'Armee'},
        townSpec5: {id: 5, title: 'Erhöhte Wichtigkeit'},
        townSpec6: {id: 6, title: 'Unersetzliche Stadt'},
        townSpec7: {id: 7, title: ''},
    },
	
    smenu: {
        cls: 'smenuIcon',
        ann: {id: 'ann', title: 'Anzeigen'},
        sim: {id: 'sim', title: 'Kampf-Simulator'},
        hlp: {id: 'hlp', title: 'Enzyklopädie'},
        opt: {id: 'opt', title: 'Einstellungen'},
        frm: {id: 'frm', title: 'Community'},
        sup: {id: 'sup', title: 'Support'},
        ext: {id: 'ext', title: 'Ausgang'},
		fsc: {id: 'fsc', title: 'Vollbild'},
    },
	...
	moneyIcon: {
		...
		inc: {id: '0', title: 'Einkommen'},
		cons: {id: '1', title: 'Ausgaben'},
		stream: {id: '2', title: 'Versorgung'},
		tax: {id: '4', title: 'Steuern'},
		budget: {id: '6', title: 'Budget-Zahlungen'},
		sum: {id: '3', title: 'Gewinn'},
	},
	...
	resProdtype: {
		...
		science: {id: 0, title: 'Wissenschaftliche'},
		money: {id: 1, title: 'Finanzen'},
		production: {id: 3, title: 'Production'},
		food: {id: 2, title: 'Landwirtschaftliche'},
	},
